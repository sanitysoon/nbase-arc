...
#ifdef _WIN32
sock_close_func g_sock_closef = NULL;
#endif
...
int zookeeper_interest(zhandle_t *zh, SOCKET *fd, int *interest,
     struct timeval *tv, sock_nonblock_func connect_func)
{
    ...
    *interest = 0;
    ...
    if (*fd == -1) {
       if (zh->connect_index == zh->addrs_count) {
            ...
            int rc;
#ifdef _WIN32
	    struct sockaddr_in *addr = &zh->addrs[zh->connect_index];
	    char conn_err[256];
	    char *zk_host;
	    DWORD host_len = 0;
	    unsigned short zk_port = 0;

	    zk_host = inet_ntoa(((struct sockaddr_in*)addr)->sin_addr);
	    zk_port = ntohs(addr->sin_port);
	    zh->fd = connect_func(conn_err, zk_host, zk_port);
            if (zh->fd == -1) {
                rc = -1;
